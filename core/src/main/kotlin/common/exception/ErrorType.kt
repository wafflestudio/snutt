...
enum class ErrorType(
    ...
    INVALID_TIME(HttpStatus.BAD_REQUEST, 0x100C, "time이 유효하지 않습니다", "강좌의 시간이 올바르게 설정되었는지 확인해 주시기 바랍니다", "시간대 겹침"),

    WRONG_API_KEY(HttpStatus.FORBIDDEN, 0x2000, "API 키가 잘못되었습니다", "API 키가 잘못되었습니다", "로그인 실패"),
    NO_USER_TOKEN(HttpStatus.UNAUTHORIZED, 0x2001, "유저 토큰이 존재하지 않습니다", "앱을 완전히 종료한 뒤, 로그인을 다시 시도해 주시기 바랍니다", "로그인 실패"),
    WRONG_USER_TOKEN(HttpStatus.FORBIDDEN, 0x2002, "유저 토큰이 유효하지 않습니다", "앱을 완전히 종료한 뒤, 로그인을 다시 시도해 주시기 바랍니다", "로그인 실패"),
    USER_NOT_ADMIN(HttpStatus.FORBIDDEN, 0x2003, "어드민 권한이 없습니다", "Admin 사용자가 아닙니다. 운영진에게 문의 주시기 바랍니다", "잘못된 요청"),
    WRONG_LOCAL_ID(HttpStatus.FORBIDDEN, 0x2004, "잘못된 id 입니다", "등록되지 않은 아이디입니다. 다시 입력해 주세요", "로그인 실패"),
    WRONG_PASSWORD(HttpStatus.FORBIDDEN, 0x2005, "잘못된 password 입니다", "잘못된 비밀번호입니다. 다시 입력해 주세요", "로그인 실패"),
    ...
    DUPLICATE_LOCAL_ID(HttpStatus.FORBIDDEN, 0x3002, "localId가 중복되었습니다", "이미 존재하는 아이디입니다", "아이디 등록 실패"),
    DUPLICATE_TIMETABLE_TITLE(HttpStatus.FORBIDDEN, 0x3003, "timetable title이 중복되었습니다", "같은 학기에 동일한 이름의 시간표가 있습니다", "시간표 이름 중복"),
    ...
    LECTURE_TIME_OVERLAP(HttpStatus.FORBIDDEN, 0x300C, "강의 시간이 겹칩니다", "시간대 겹침"),
    ...
    INVALID_NICKNAME(HttpStatus.BAD_REQUEST, 40008, "nickname이 유효하지 않습니다", "사용할 수 없는 닉네임입니다", "올바르지 않은 닉네임"),
    ...
    INVALID_TIMETABLE_ORDER_REQUEST(
        HttpStatus.BAD_REQUEST,
        40028,
        "시간표 순서 변경 요청이 유효하지 않습니다",
        "시간표 순서 변경 요청이 유효하지 않습니다",
    ),
