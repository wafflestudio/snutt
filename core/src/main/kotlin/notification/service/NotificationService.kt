...
class NotificationService internal constructor(
    ...
    suspend fun getNotifications(query: NotificationQuery): List<NotificationResponse> {
        val user = query.user
    ...
    suspend fun sendPush(userId: String, pushMessage: PushMessage) {
        saveNotification(userId, pushMessage)
    ...
    suspend fun sendPushes(userIds: List<String>, pushMessage: PushMessage) {
        saveNotifications(userIds, pushMessage)
    ...
    suspend fun sendGlobalPush(pushMessage: PushMessage) {
        saveNotification(userId = null, pushMessage)

        pushClient.sendGlobalMessage(pushMessage)
    }
